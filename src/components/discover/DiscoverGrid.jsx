...
export default function DiscoverGrid({ category, genre, mediaTypeFilter = 'all', selectedNetwork = null, selectedStudio = null, existingMovies, existingSeries, requests }) {
    ...
    return (
      <div className="flex flex-col items-center justify-center py-16 text-center">
        ...
        <p className="text-sm font-medium mb-1">
          {needsKey ? 'TMDB API key required' : 'Failed to load from TMDB'}
        </p>
        <p className="text-xs text-muted-foreground max-w-md mb-4">
          {needsKey
            ? 'Add a free TMDB v3 API key in Settings → General to enable Discover.'
  ...
  const movieItems = enrichedItems.filter(i => i.media_type === 'movie');
  const seriesItems = enrichedItems.filter(i => i.media_type === 'series');

  return (
    <>
      {enrichedItems.length === 0 ? (
        <EmptyState icon={Compass} title="No results" description="Try a different category or filter" />
      ) : (
        <>
          {movieItems.length > 0 && (
            <>
